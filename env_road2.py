...
#(x,y,direcSin,direcCos)
    ...
    def step(self, action):
        ...
        elif not self.inside(self.currentPos[0],self.currentPos[1]):
            done = True
            R += 0
        ...
        # going from top to bottm at the second half
            ...
            if self.inr1(self.currentPos[0],self.currentPos[1]):
                R += min((stepStartingPos[1] - self.currentPos[1])/ self.YSIZE,2*(stepStartingPos[1] - self.currentPos[1])/ self.YSIZE)

            if self.inr2(self.currentPos[0],self.currentPos[1]):
                R += min((self.currentPos[0] - stepStartingPos[0])/ self.XSIZE,2*(self.currentPos[0] - stepStartingPos[0])/ self.XSIZE)

            if self.inr3(self.currentPos[0],self.currentPos[1]):
                R += min((self.currentPos[1] - stepStartingPos[1])/ self.YSIZE,2*(self.currentPos[1] - stepStartingPos[1])/ self.YSIZE)

            if self.inr4(self.currentPos[0],self.currentPos[1]):
                R += min((stepStartingPos[0] - self.currentPos[0])/ self.XSIZE,2*(stepStartingPos[0] - self.currentPos[0])/ self.XSIZE)
            done = False
        if self.counter >= 300:
            done = True
        else:
